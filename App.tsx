  ...
  render() {
    ...
    return (
      <View style={styles.container} >
        ...
        <PlayerControls inverseOrder={false} transformRotateZ={'180deg'} gameState={this.state.game} handlePlayerAction={(action) => this.onPlayerAction(action, players[0])} />

        <PlayerStatistics transformRotateZ={'180deg'} GameState={this.state.game} />

        <TableModul game={this.state.game} handleCardClicked={() => this.startGame()} card={this.state.card} />

        <PlayerStatistics transformRotateZ={'0deg'} GameState={this.state.game} />

        <PlayerControls inverseOrder={false} transformRotateZ={'0deg'} gameState={this.state.game} handlePlayerAction={(action) => this.onPlayerAction(action, players[1])} />
